  ...
  return (
    <footer className="h-6 bg-ps-header border-t border-ps-border flex items-center justify-between px-3 text-[11px] text-zinc-400 select-none z-30">
      ...
      <div className="flex items-center space-x-3">
        ...
        <div className="flex items-center space-x-1.5 border-l border-ps-border/70 pl-3 font-mono text-[10px]">
          <PenTool size={11} className={isPen ? 'text-blue-400 animate-pulse' : 'text-zinc-500'} />
          {isPen ? (
            <div className="flex items-center space-x-2 text-zinc-300">
              <span className="text-blue-400 font-medium">
                {tabletTelemetry.isEraser ? 'Stylus Eraser' : 'Tablet Stylus'}
              </span>
              <div className="flex items-center space-x-1 bg-zinc-800 px-1.5 py-0.2 rounded border border-zinc-700">
                <span>P:</span>
                <span className="text-emerald-400 font-bold">{pressurePercent}%</span>
                {/* Mini pressure gauge bar */}
                <div className="w-8 h-1.5 bg-zinc-700 rounded-full overflow-hidden ml-1">
                  <div
            ...
            <span className="text-zinc-500">Tablet: Ready</span>
          )}
        </div>
      </div>
